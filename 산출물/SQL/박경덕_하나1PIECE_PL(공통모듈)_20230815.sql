...
CREATE OR REPLACE PACKAGE BODY HANA1PIECE_MNG AS
    ...
    BEGIN
        ...
        SELECT WALLET_NUMBER, 
               P_LISTING_NUMBER, 
               AMOUNT * P_PAYOUT AS TOTAL_PAYOUT
        FROM STOS 
        WHERE LISTING_NUMBER = P_LISTING_NUMBER AND AMOUNT > 0;
        ...
        ON (w.WALLET_NUMBER = s.WALLET_NUMBER)
        WHEN MATCHED THEN
            UPDATE SET w.BALANCE = w.BALANCE + s.TOTAL_PAYOUT;
    
        -- 지갑 거래내역 기록
        INSERT INTO WALLET_TRANSACTION(WALLET_NUMBER, CLASSIFICATION, NAME, AMOUNT, BALANCE)
        SELECT s.WALLET_NUMBER, 
               'IN', 
               P_LISTING_NUMBER || ' 배당금', 
               s.AMOUNT * P_PAYOUT, 
               w.BALANCE + (s.AMOUNT * P_PAYOUT)  -- Assuming this calculates the new balance after the payout
        FROM STOS s
        JOIN WALLET w ON s.WALLET_NUMBER = w.WALLET_NUMBER
        WHERE s.LISTING_NUMBER = P_LISTING_NUMBER AND s.AMOUNT > 0;
    
        COMMIT;
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            WRITE_LOG('ERR:PD', 'PAY_DIVIDEND2', SQLERRM, '배당금 지급 실패');
    END PAY_DIVIDEND2;
